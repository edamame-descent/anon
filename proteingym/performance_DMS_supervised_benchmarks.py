...
def compute_bootstrap_standard_error_functional_categories(df, number_assay_reshuffle=10000, top_model="ProteinNPT"):
    ...
    for model_name, group in tqdm(df.groupby("model_name")):
        ...
        model_errors[model_name] = combined_averages.std(ddof=1) #Unbiased estimate with ddof=1
    # mean_performance_across_samples = mean_performance_across_samples.mean(axis=1)
    return pd.DataFrame(model_errors).transpose() #Unbiased estimate with ddof=1
...
if __name__ == "__main__":
    ...
    # Some inconsistency in capitalization of MSA_Neff_L_category
