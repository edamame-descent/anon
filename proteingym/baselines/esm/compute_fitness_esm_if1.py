...
def get_sequence_loss_batch(model, alphabet, coords_list, seq_list):
    # Similar to utils.get_sequence_loss, but in a batch
    ...
    # print(f"Debug coords: {coords.shape}, tokens: {tokens.shape}, padding_mask: {padding_mask.shape}")
    prev_output_tokens = tokens[:, :-1].to(device)
    target = tokens[:, 1:]
    target_padding_mask = (target == alphabet.padding_idx)
    logits, _ = model.forward(coords, padding_mask, confidence, prev_output_tokens)
    loss = F.cross_entropy(logits, target, reduction='none')
    # print(f"Debug logits: {logits.shape}, loss: {loss.shape}")
...
def score_sequence_batch(model, alphabet, coords_list, seq_list):
    ...
    # ll_fullseqs = -np.sum(losses * ~target_padding_masks) / np.sum(~target_padding_masks)
...
def score_singlechain_backbone_batch(model, alphabet, pdb_file, chain, mutation_file, output_filepath, batch_size=1,nogpu=False):
    # A copy of score_singlechain_backbone but using a batch
    # Notes: 
    # - Can batch up a file into batch_size sequences using this function, at the moment everything goes into 1 big batch
    # - Should probably do this in a dataloader
    if not nogpu:
        assert torch.cuda.is_available(), "Expected GPU. If you want to use CPU, you have to specify --nogpu every time."
        model = model.cuda()  # Takes about 1GB of GPU memory
        print("Transferred model to GPU")
    else:
        print(f"Running model on CPU: torch cuda is available={torch.cuda.is_available()} nogpu={nogpu}")

    # Scoring multiple sequences on the same PDB file for now (not multiple PDB files and multiple seq files... although that's easy to extend to)
    ...
    coords_list = [coords] * len(seq_list) # Reusing the same coords each time, so copying num_seqs times
...
def main():
    # NOTE: Currently only works with small batch sizes, default is 1. Throws a very weird cuda error for large batches
