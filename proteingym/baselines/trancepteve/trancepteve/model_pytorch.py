...
class TrancepteveLMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.inference_time_retrieval_type is not None:
            ...
            #self.to(self.default_model_device)
            ...
            if retrieval_aggregation_mode is not None:
                ...
                for seq_index in range(batch_size):
                    ...
                    if inference_time_retrieval_type=="TranceptEVE" and self.config.MSA_threshold_focus_cols_frac_gaps<1.0 and retrieval_aggregation_mode!="aggregate_indel":
                        # If the retrieved EVE model filters positions due to coverage, we fully rely on Tranception at these positions (including retrieved MSA since we do not filter that MSA based on coverage)
                        # Note: does not support indels
                        # flip indices as needed
                        #if flip[seq_index]:
                        #    reindexed_non_focus_cols = [self.full_protein_length - position_index_full_protein - 1 for position_index_full_protein in self.EVE_MSA.non_focus_cols]
                        #else:
                        #    reindexed_non_focus_cols = self.EVE_MSA.non_focus_cols
                        ##Reindexing based on slice start
                        #reindexed_non_focus_cols = [position_index_full_protein - start_slice[seq_index]  for position_index_full_protein in reindexed_non_focus_cols]
                        ##Filtering positions that are not in MSA/EVE and Transformer aggregation window
                        #reindexed_non_focus_cols = [position for position in reindexed_non_focus_cols if (position >= min_logits_slice and position < max_logits_slice)]

                        #Alternative method - just check where inf values are:
                ...
                # Underneath is only used at train time -- inference logic in scoring_utils
