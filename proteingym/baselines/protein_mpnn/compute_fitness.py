...
def main(args):
    ...
    with torch.no_grad():
        ...
        for ix, protein in enumerate(dataset_valid):
            ...
            # this is the score only condition of the original script. Since we only deal with fitness scores in ProteinGym I've cut the rest out for clarity
            loop_c = 0 
            fasta_names, fasta_seqs = dms_df["mutant"].tolist(), dms_df["mutated_sequence"].tolist()
            loop_c = len(fasta_seqs)
        # This originall scored the wild type sequence as well, but now changing to only score mutants to match other methods
        ...
        #         structure_sequence_score_file = base_folder + '/score_only/' + batch_clones[0]['name'] + f'_fasta_{fc}'
            ...
            for fc in range(loop_c):
                ...
                # negated here because _scores gives back negative log probabilities and we want higher scores to be better
